...
A **Galton board** (named after [Sir Francis Galton](https://en.wikipedia.org/wiki/Francis_Galton), a pioneering statistician and, notably, a eugenicist, which is unfortunately a recurring theme with leading early statisticians...), shown below, is a physical device that demonstrates the Central Limit Theorem. A Galton board has rows of pins arranged in a triangular shape, with each row of pins offset horizontally relative to the rows above and below it. As illustrated in @fig-galton, the top row has one pin, the second row has two pins, and so forth. If you've watched *The Price is Right*, you've seen this as ["Plinko"](https://priceisright.fandom.com/wiki/Plinko).
...
#| output: true
#| label: fig-galton
#| fig-cap: "Galton Board simulation. The top shows a schematic of how the Galton board works and the bottom shows a histogram of the landing positions after 200 simulations."
#| fig-alt: "Galton Board simulation"
#| fig-align: center

function draw_board(nlevels, width, height)
    # set geometry of board
    yoffset = height / (nlevels - 1)
    xpos = 1
    ypos = 0
    # initialize location of galton board pins
    pins = Array{Tuple{Float64, Float64}}(undef, 0)

    # in each level, create pins
    for level in nlevels:-1:1
        for i=1:level
            push!(pins, (xpos + (i - 1), ypos))
        end
        xpos += 1 / 2
        ypos += yoffset
    end
    ...
    # draw pins
    p1 = scatter(pins, markershape=:dtriangle, markersize=10, color=:blue, legend=:false, xaxis=:false, yaxis=:false, grid=:false)
    xlims!(0, nlevels + 1)
    ylims!((-10, height+17))
    ...
    for i = 1:(nlevels+1)
        ...
        scatter!((ball_x, ball_y + 11), markershape=:circle, color=:red, markersize=7, alpha = ball_alpha)
        
        bounce =  sample([-0.5, 0.5])
        if i < nlevels
            ball_x = ball_x + bounce
            ball_y = ball_y - yoffset
        else
            ball_x = ball_x + bounce
            ball_y = ball_y - yoffset 
        end
...
function plot_galton_hist(nballs, nlevels, p=0.5)
    function simulate_ball(nlevels, p)
        bin = 0
        for i = 1:nlevels
            bin = bin + sample([0, 1], ProbabilityWeights([1-p, p]))
        end
        return bin
    end
    bins = [simulate_ball(nlevels, p) for i in 1:nballs]
    p = histogram(bins, bins=0:1:nlevels+1, legend=:false, grid=:false, yaxis=:false, xaxis=:false, ticks=:false)
    xlims!(0, nlevels + 1)
    ylims!(0, Inf)
    vline!(collect(0:1:(nlevels+1)), color=:black, linewidth=2)
    hline!([0], color=:black, linewidth=2)
    return p
end

p1 = draw_board(8, 100, 100)
p2 = plot_galton_hist(200, 8)
plot(p1, p2, layout=grid(2, 1, heights=[0.6, 0.4]))
```
...
[![Source: XKCD #2659](https://imgs.xkcd.com/comics/unreliable_connection_2x.png)](https://xkcd.com/2659/){#fig-xkcd fig-align="center"}
